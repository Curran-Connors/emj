<meta name="robots" content="noindex,nofollow">

<meta name="viewport" content="width=device-width, initial-scale=1.0">

<!-- FONTS -->
<link href='http://fonts.googleapis.com/css?family=Noto+Sans:400,700,400italic,700italic' rel='stylesheet' type='text/css'>

<!-- CSS test -->
<link href="{site_url}style.css" type="text/css" rel="stylesheet">
<link href="{site_url}css/responsive.css" type="text/css" rel="stylesheet">
<link href="{site_url}css/browsers.css" type="text/css" rel="stylesheet">
<link href="{site_url}css/devices.css" type="text/css" rel="stylesheet">

<!-- DEVELOPER STYLESHEETS -->
<!-- ** PLEASE USE any of the 4 stylesheets below to avoid overriding eachother. Developer1.css is uncommented as first default. Uncomment whichever one you'd like to use as well if needed -->
<link href="{site_url}css/developer1.css" type="text/css" rel="stylesheet">
<!--<link href="css/developer2.css" type="text/css" rel="stylesheet">
<link href="css/developer3.css" type="text/css" rel="stylesheet">
<link href="css/developer4.css" type="text/css" rel="stylesheet">-->

<!-- Bootstrap core CSS -->
<link href="{site_url}bootstrap/css/bootstrap.min.css" rel="stylesheet">

<!-- PLUGINS CSS -->
<?php $plugins->load_plugin_css(); ?>


<!--[if IE 9]>
	<link href="{site_url}css/ie9.css" type="text/css" rel="stylesheet">
<![endif]-->

<!--[if IE 7]>
	<link href="{site_url}css/ie7.css" type="text/css" rel="stylesheet">
<![endif]-->

<!-- HTML5 shim and Respond.js IE8 support of HTML5 elements and media queries -->
<!--[if lt IE 9]>
  	<script src="https://oss.maxcdn.com/libs/html5shiv/3.7.0/html5shiv.js"></script>
  	<script src="https://oss.maxcdn.com/libs/respond.js/1.3.0/respond.min.js"></script>
	<link href="{site_url}css/ie.css" type="text/css" rel="stylesheet">
<![endif]-->

<!-- MODERNIZR -->
<script type="text/javascript" src="{site_url}js/modernizr.js"></script>
<script type="text/javascript"
      src="https://maps.googleapis.com/maps/api/js?key=AIzaSyAVq6EjFY26ze6w1nPKnq6un3CrYCzLo0E">
    </script>
<!-- Favicons -->
<link rel="apple-touch-icon-precomposed" sizes="144x144" href="#">
<link rel="shortcut icon" href="{site_url}favicon.ico">

<!-- GOOGLE ANALYTICS -->