...
class Plugin {
	...
	public function css($path_to_file) {
		return '<link rel="stylesheet" href="{site_url}'.$this->p_dir.'/'.$path_to_file.'" type="text/css" media="screen" charset="utf-8" />
';
	}
	public function css_ext($path_to_file) {
		return '<link rel="stylesheet" href="{site_url}'.$path_to_file.'" type="text/css" media="screen" charset="utf-8" />
';
	}
	
	public function js($path_to_file) {
		return '<script src="{site_url}'.$this->p_dir.'/'.$path_to_file.'" type="text/javascript" charset="utf-8"></script>
';
	}
	public function js_ext($path_to_file) {
		return '<script src="{site_url}'.$path_to_file.'" type="text/javascript" charset="utf-8"></script>
';
	}
	...
	****************************************************************/
	public function add_plugin($plugin_array) {
		foreach($plugin_array as $plugin) {
			...
			// LIST BLOCK
			if($plugin == 'listblock') {
				$dir = 'listblock'; // This is the name of the specific plugin directory
				$this->plugins_css[] 	= self::css($dir.'/listblock.css'); // The path to the CSS file
				$this->plugins_js[]		= self::js($dir.'/listblock.js'); // The path to the JS file
				
				// REFERENCE: 	Made by yours truly 
				/* HELPERS: 	
				*/			   	
			}
			
			// TICKET SLIDER
			if($plugin == 'ticket_slider') {
				$dir = 'ticket-slider'; // This is the name of the specific plugin directory
				$this->plugins_css[] 	= self::css($dir.'/ticket-slider.css'); // The path to the CSS file
				$this->plugins_js[]		= self::js($dir.'/ticket-slider.js'); // The path to the JS file
